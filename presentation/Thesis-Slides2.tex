...
\section{Introdução}
%------------------------------------------------
\begin{frame}{Decisão de escalonamento}
Em cada TTI, a estação base escolhe um UE para receber o recurso disponível.

\begin{columns}[T,onlytextwidth]
\begin{column}{0.30\textwidth}
\begin{block}{Round Robin (RR)}
\[u_t=t\bmod N\]
\small Alterna os UEs em ordem cíclica, ignorando a taxa do canal.
\end{block}
\end{column}
\begin{column}{0.30\textwidth}
\begin{block}{Max C/I (MaxCI)}
\[u_t=\arg\max_u r_{u,t}\]
\small Escolhe a maior taxa instantânea (política max-rate).
\end{block}
\end{column}
\begin{column}{0.30\textwidth}
\begin{block}{Proportional Fair (PF)}
\[u_t=\arg\max_u \frac{r_{u,t}}{T_u(t)}\]
\small Compara taxa instantânea e histórico de vazão ($\beta=0{,}98$).
\end{block}
\end{column}
\end{columns}

\vspace{1mm}
\begin{alertblock}{Comparação pareada}
Para cada seed, as três políticas recebem a mesma realização de canal.
\end{alertblock}
\end{frame}

\begin{frame}{Pergunta do estudo}
...
\begin{itemize}
    \item \textbf{Carga}: $N\in\{2,4,8,16,32\}$ UEs;
    \item \textbf{Canal}: UEs homogêneos ou ganho relativo log-distance com $\alpha\in\{1{,}5,\ldots,4{,}0\}$;
    \item \textbf{Métrica}: vazão agregada, justiça de slots e vazão, cauda inferior e exclusão.
\end{itemize}

\vspace{2mm}
\begin{alertblock}{Hipótese avaliada}
...
\begin{frame}{Motivação}
    \begin{itemize}
        ...
        \item A heterogeneidade do canal pode favorecer usuários com melhores condições, em detrimento de outros. Uma política que funciona bem em cenários homogêneos \textbf{pode falhar em cenários heterogêneos}.

        \item É importante analisar o \textbf{compromisso entre eficiência e justiça} entre diferentes políticas de escalonamento.

    \end{itemize}

    \begin{block}{Objetivo} Avaliar como carga e heterogeneidade do canal afetam esse compromisso em RR, MaxCI e PF.
    \end{block}
\end{frame}
...
\begin{frame}{Condução do Experimento}
    ...
    \begin{itemize}
        ...
        \item TTI de 1 ms; banda ocupada de 18,72 MHz; SNR de referência de 10 dB; teto de 6 bit/s/Hz.
...
\begin{frame}{Cenário Heterogêneo}
    ...
    \begin{itemize}
        \item \( \tilde d_u = \max(d_u, d_0) \), com \( d_u \) a distância à estação base e \( d_0 = 10 \) m;
...
\begin{itemize}
    ...
    \item \textbf{MaxCI} mantém justiça agregada alta no cenário homogêneo; por simetria, todos os UEs têm a mesma chance de serem selecionados.

    \item Maior intervalo interno sem atendimento (média entre seeds):
...
\end{itemize}
    ...
    \begin{block}{} A diferença pareada PF menos RR no JFI de vazão foi de 0,140, com IC95 normal aproximado de ±0,012 (o intervalo não inclui zero).
    \end{block}

\end{frame}
%------------------------------------------------

\begin{frame}{Justiça de slots e de vazão}

\begin{figure}[htp!]
\centering
\includegraphics[width=0.95\linewidth]{figures/jfi_cenarios.png}
\caption{Comparação entre o cenário homogêneo e o log-distance $\alpha=3$. MaxCI perde justiça com a carga apenas no cenário heterogêneo.}
\end{figure}
\end{frame}
...
\textbf{Cenário representativo} (N=32, $\alpha=3$)
\begin{table}[htbp]
\centering
\small
...
\hline
Política & JFI slots & JFI vazão & Vazão célula (Mbit/s) & P5 vazão (Mbit/s) & UEs excluídos \\
\hline
...
\begin{frame}{Compromisso em $N=32$}

\begin{figure}[htp!]
\centering
\includegraphics[width=0.9\linewidth]{figures/tradeoff_n32.png}
\caption{MaxCI maximiza a vazão agregada ao custo da justiça de vazão; PF equilibra as duas.}
\end{figure}
\end{frame}


%------------------------------------------------
\begin{frame}{Severidade da heterogeneidade}

\begin{figure}[htp!]
\centering
\includegraphics[width=0.9\linewidth]{figures/maxci_alpha.png}
\caption{Para cada carga, aumentar $\alpha$ reduz o JFI de slots e eleva o Gini do MaxCI.}
\end{figure}
\end{frame}

\begin{frame}{Robustez à métrica de justiça}

\begin{figure}[htp!]
\centering
\includegraphics[width=0.9\linewidth]{figures/robustez_metricas.png}
\caption{Em $N=32$ e $\alpha=3$, MaxCI permanece abaixo de RR e PF em toda a família $J_\beta$; RR e PF ficam próximos de 1.}
\end{figure}
\end{frame}
...
\begin{frame}{Conclusão}
     \begin{itemize}
         ...
         \item O \textbf{PF} apresentou, entre as políticas avaliadas, o melhor equilíbrio entre \textbf{vazão e justiça} no cenário heterogêneo.
    \end{itemize}
    \vspace{0.3cm}
    \begin{block}{Conclusão principal} A escolha do escalonador deve considerar não apenas a vazão, mas também a heterogeneidade do canal e a métrica de justiça adotada.
    \end{block}
\end{frame}
...
Usado como referência de contexto; o estudo não implementa o cenário completo do TR 38.901.
